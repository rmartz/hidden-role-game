...
- Both rules — OKF frontmatter and index reachability — are enforced in CI by `pnpm run check:docs` (`scripts/check-docs.mjs`), which runs via the `Docs` workflow on any PR that changes a `docs/` page (or the checker itself).
