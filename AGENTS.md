...
- **Every page must be reachable from the top-level index (`docs/README.md`) by following links.** A page is "reachable" when an index file (`README.md` / `index.md`) links to it, or links to a sub-index that (transitively) links to it — so a reader can navigate `docs/README.md` → sub-index → page. Add a link from the appropriate index whenever you add a page; an orphan page that no index reaches is a violation.
