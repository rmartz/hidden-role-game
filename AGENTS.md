...
- **Pin every dependency to a full `major.minor.patch` version**, including the range annotation — e.g. `"prettier": "^3.8.4"`, never `"^3"` or `"^3.8"`. A partial pin lets Dependabot bump the package without changing `package.json` (only `pnpm-lock.yaml` moves), so the update is invisible in the PR diff and a version bump that requires code or formatting changes looks like it "appeared from nowhere". Full pins make every bump explicit in `package.json`. This applies to registry version ranges only; non-registry specifiers (tarball URLs, `git+`, `workspace:`, `file:`, `link:`, dist-tags) are exempt. Enforced in CI by the `package-pins` check in [`@rmartz/repo-hygiene`](https://github.com/rmartz/repo-hygiene) (the `Repo Hygiene` workflow, configured in `.repo-hygiene.yml`).
...
- **Source files**: Keep under ~200 lines (split at ~240). Large files should be split by logical concern. The eslint `max-lines` rule enforces two tiers (`pnpm lint` / editor / CI): a **soft warn at 240** (advisory — surfaces in the editor and CI logs but does not fail the build) and a **hard error cap at 400** (fails lint/CI). Aim well under the warn tier; the error cap is the ceiling.
- **Test files**: Keep under ~300 lines (split at ~360). Same two-tier `max-lines`: **soft warn at 360**, **hard error cap at 600**. Use `.spec.ts` / `.spec.tsx` extension (not `.test.ts`). When splitting, organize into a `{module}-tests/` directory with domain-specific files (e.g., `resolution-tests/altruist.spec.ts`).
...
- **A game mode's logic internals are sealed.** Code outside a mode (`src/lib/game/modes/<mode>/`) — components, app routes, server, the firebase/store layers — must **not** import that mode's `utils/` or `actions/` internals. Consume the mode's public surface instead: its barrel (`@/lib/game/modes/<mode>`, which already re-exports what's public) or its public data modules (`types`, `copy`, `roles`, `*-config`, `player-state`, `themes`). A rule threshold constant lives with the logic that owns it (in its action file) and is re-exported through the barrel, so a UI that mirrors the rule imports it from the barrel — not from the action. This keeps a logic refactor's blast radius inside the mode. Enforced by a `no-restricted-imports` rule in `eslint.config.js` (`pnpm lint` / editor / CI). (The mode's _data_ surface is intentionally imported directly and is not restricted; a broader barrel-only migration is tracked separately.)
...
- Content pages under `docs/` follow the [Open Knowledge Format (OKF)](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md): every content page begins with YAML frontmatter. The required key is `type` (one of `Guide`, `Reference`, `Roles`, `Actions`, `DataFlow`); also set `title` and `description`. Per-mode pages additionally set `gameMode` (the mode slug) and `resource` (relative path to the documented source). When you add a new content page, include the frontmatter and link it from `docs/index.md` so the index stays complete.
- **The directory index is `index.md`** (the OKF canonical index filename, not `README.md`). The top-level index is `docs/index.md`; a subdirectory that needs its own index uses `<dir>/index.md`. Per OKF §8/§11 the reserved `index.md` is **exempt from the frontmatter requirement**: it carries no frontmatter, with the single exception that it MAY carry an `okf_version` key (`docs/index.md` uses only `okf_version: "0.2"`). Any other frontmatter key on an `index.md` is rejected.
- **Every page must be reachable from the top-level index (`docs/index.md`) by following links.** A page is "reachable" when an `index.md` links to it, or links to a sub-`index.md` that (transitively) links to it — so a reader can navigate `docs/index.md` → sub-index → page. Add a link from the appropriate `index.md` whenever you add a page; an orphan page that no index reaches is a violation.
- Both rules — OKF frontmatter and index reachability — are enforced in CI by the `okf` and `okf-index` checks in [`@rmartz/repo-hygiene`](https://github.com/rmartz/repo-hygiene) (the `Repo Hygiene` workflow, configured in `.repo-hygiene.yml`). Note: `okf` validates `type` / `title` / `description` / `resource`; the per-mode `gameMode` field remains a required convention but is no longer separately CI-validated.
...
- Enforced in CI by the `md-pairing` check in [`@rmartz/repo-hygiene`](https://github.com/rmartz/repo-hygiene) (the `Repo Hygiene` workflow, configured in `.repo-hygiene.yml`).
...
- **Pin every GitHub Action to a commit SHA with a version comment**, never a mutable tag — e.g. `uses: actions/checkout@9c091bb… # v7.0.0`, never `@v7`. A tag can be force-repointed at a malicious commit (the tj-actions/changed-files class of supply-chain attack); an immutable commit SHA cannot. The `# vX.Y.Z` comment is required, not decorative: Dependabot's github-actions ecosystem reads it to know which release the SHA maps to and bumps the SHA **and** comment together, so a missing comment silently strands the pin. Local composite actions (`uses: ./…`) are exempt. Enforced in CI by the `action-pins` check in [`@rmartz/repo-hygiene`](https://github.com/rmartz/repo-hygiene) (the `Repo Hygiene` workflow, configured in `.repo-hygiene.yml`), which scans all of `.github/**` including composite actions.
