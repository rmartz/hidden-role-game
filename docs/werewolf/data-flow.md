...
| `morticianResult`           | Mortician                                           | `{ targetPlayerId: string; roleName: string \| null }` — the target and their role on a successful kill; `roleName` is `null` when the attack was blocked. If protected, Mortician always receives "not a Werewolf" regardless of the target's actual team. |
...
When the Old Man role is in play, an `OLD_MAN_TIMER_KEY` is tracked in the resolution options. The timer counts down each night and is set to `(#werewolves + 1)` at game start. During `start-day` resolution, if the timer reaches zero, the Old Man dies peacefully — but only if the Old Man was **not** also attacked that same night. If the Old Man was attacked and killed by wolves (or any other attack), the attack takes precedence and no `"peaceful"` event is emitted. The `oldManTimerPlayerId` option identifies the player assigned the Old Man role for the resolution check.
