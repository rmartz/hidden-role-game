...
| Veteran       | `werewolf-veteran`       | Good    | Every Night               | None (self-alert)      | Each night may go on Alert (max 3 times). While Alerted: repels any wolf attack (one wolf dies), and counter-kills any solo player who physically visits them (Protect and Attack roles, etc.). Investigation roles (Seer, etc.) are unaffected. Priest wards do not trigger the counter-kill. |
