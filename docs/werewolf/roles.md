...
| Tavern Keeper | `werewolf-tavern-keeper` | Good    | Every Night (first)       | Special (role-block)  | Wakes before all other roles; chooses one player to serve too many drinks. The chosen player cannot perform their night action that night and sees a "too drunk" flavour message. Unique; `preventSelfTarget`. |
...
5. The **Tavern Keeper** is always moved to the **very first** position in the night order (before all category-sorted roles), so they block before anyone else acts.
6. The Witch acts **second-to-last**, after all other roles except the Altruist, so she can see current attacks before deciding.
7. The Altruist always acts **last**, after the Witch, so it can intercept attacks (including Witch attacks) on its target.
