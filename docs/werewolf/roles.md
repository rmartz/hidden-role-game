...
| Arsonist      | `werewolf-arsonist`      | Neutral | Every Night               | Special (douse/ignite) | Targeting another player *douses* them (persistent mark). Targeting self *ignites* — all currently doused players are simultaneously attacked (protections apply individually); doused list resets after ignite. Wins as the last opposing threat. Seer reads "not a werewolf". |
