...
| Evil Empath     | `werewolf-evil-empath`     | Bad     | Every Night               | Special (auto-result)  | Each night, the Narrator confirms whether the Seer is seated adjacent (circular) to any living Werewolf; result stored in `roleState.evilEmpath.lastResult`. When the Evil Empath dies (night or day), the last result is revealed to all Werewolves as `roleState.evilEmpath.revealedResult`.                                                                                  |
...
| Illusion Artist | `werewolf-illusion-artist` | Bad     | After First Night         | Special (invert)       | Selects one target per night; if the Seer investigates that player, the investigation result is inverted (guilty→innocent, innocent→guilty). Cannot target the same player on consecutive nights (`preventRepeatTarget`). Cannot target self.                                                                                                                                   |
...
| Seer            | `werewolf-seer`            | Good    | Every Night               | Investigate            | Learns whether the target is a Werewolf (`isWerewolf` flag); non-wolf Bad-team roles (Minion, Wizard) read as innocent. Narrator reveals result                                                                                                                                                                                                                                 |
