...
| Veteran       | `werewolf-veteran`       | Good    | Every Night               | None (self-alert)     | Each night may go on Alert (max 3 times). While Alerted: repels any wolf attack (one wolf dies), and kills any visiting Protect-role player. Priest wards do not trigger the counter-kill. |
