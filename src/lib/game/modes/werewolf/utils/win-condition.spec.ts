...
describe("checkWinCondition", () => {
  ...
  describe("Dracula", () => {
    it("Dracula wins when alive and has 3 wives alive", () => {
      const ts = makeDayTurnState({ draculaWives: ["p2", "p3", "p4"] });
      const game = makeGame(
        [
          { playerId: "p1", roleDefinitionId: WerewolfRole.Dracula },
          { playerId: "p2", roleDefinitionId: WerewolfRole.Villager },
          { playerId: "p3", roleDefinitionId: WerewolfRole.Villager },
          { playerId: "p4", roleDefinitionId: WerewolfRole.Villager },
      ...
      const result = checkWinCondition(game, []);
      expect(result?.winner).toBe(WerewolfWinner.Dracula);
    });

    it("game continues when Dracula has only 2 wives alive", () => {
      const ts = makeDayTurnState({ draculaWives: ["p2", "p3"] });
      const game = makeGame(
        [
          { playerId: "p1", roleDefinitionId: WerewolfRole.Dracula },
          { playerId: "p2", roleDefinitionId: WerewolfRole.Villager },
      ...
      const result = checkWinCondition(game, []);
      expect(result?.winner).not.toBe(WerewolfWinner.Dracula);
    });

    it("game continues when Dracula has 3 wives but one is dead", () => {
      const ts = makeDayTurnState({ draculaWives: ["p2", "p3", "p4"] });
      const game = makeGame(
        [
          { playerId: "p1", roleDefinitionId: WerewolfRole.Dracula },
      ...
      // p4 is dead → only 2 alive wives
      const result = checkWinCondition(game, ["p4"]);
      expect(result?.winner).not.toBe(WerewolfWinner.Dracula);
    });

    it("Dracula does not win if Dracula is dead", () => {
      const ts = makeDayTurnState({ draculaWives: ["p2", "p3", "p4"] });
      const game = makeGame(
        [
          { playerId: "p1", roleDefinitionId: WerewolfRole.Dracula },
